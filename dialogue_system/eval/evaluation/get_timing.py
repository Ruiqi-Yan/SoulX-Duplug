...
def process_folder(fld: Path):
    ...
    avg_stop_latency = (
        sum([e - s for s, e in data["latency_stop_list"]])
        / len(data["latency_stop_list"])
        if len(data["latency_stop_list"])
        else 0
    )
    avg_resp_latency = (
        sum([e - s for s, e in data["latency_resp_list"]])
        / len(data["latency_resp_list"])
        if len(data["latency_resp_list"])
        else 0
    )

    if avg_stop_latency > 0:
        record_stop.append(avg_stop_latency)
    if avg_resp_latency > 0:
        record_resp.append(avg_resp_latency)

    print(f"stop latency: {avg_stop_latency:.3f}s")
    print(f"respond latency: {avg_resp_latency:.3f}s")
...
def main():
    ...
    print("\n=== Summary ===")
    overall_stop = (sum(record_stop) / len(record_stop)) if record_stop else 0
    overall_resp = (sum(record_resp) / len(record_resp)) if record_resp else 0
    print(f"Overall stop latency: {overall_stop:.3f}s")
    print(f"Overall respond latency: {overall_resp:.3f}s")
